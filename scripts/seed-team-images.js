...
// Full hardcoded data with CORRECT image paths from public/images/CoreTeam
const coreTeam = [
...
// Log to file helper
const logStream = fs.createWriteStream('seed-images-log.txt', { flags: 'w' });
...
async function seed() {
  try {
    ...
    for (let i = 0; i < coreTeam.length; i++) {
        ...
        let imageUrl = member.image // Default fallback
        ...
        try {
          ...
          const uploadResult = await cloudinary.uploader.upload(base64Image, {
              ...
              resource_type: 'auto',
              public_id: member.name.toLowerCase().replace(/\s+/g, '_')
          })
    ...
    console.log(`\n🎉 Seeding complete! ${successCount} uploaded, ${errorCount} errors.`)
